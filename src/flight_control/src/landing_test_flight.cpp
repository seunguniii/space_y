...
class LandingTest : public rclcpp::Node {
  public:
    LandingTest() : Node("landing") {
      ...
      this->declare_parameter<int>("start_mode_param", 0); //0: start from ground   1: start from mid-flight
      ...
      auto timer_callback = [this]() -> void {
        ...
        if(!armed_ && mission_mode_ != FINISHED) {
          start_mode_ = this->get_parameter("start_mode_param").as_int();

          if(start_mode_ == 0) mission_mode_ = FLIGHT;
          else mission_mode_ = LANDING;
