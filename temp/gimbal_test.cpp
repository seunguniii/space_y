...
class LandingTest : public rclcpp::Node {
  public:
    LandingTest() : Node("landing") {
      ...
      gimbal_set_attitude_publisher_ = this->create_publisher<GimbalManagerSetAttitude>("fmu/out/gimbal_manager_set_attitude", 10);
  ...
  if(armed_) {
    ...
    if(dist < 3.0f) {
      ...
      if(hold_counter_ > HOLD_THRESHOLD) {
        ...
        //give ros node access to gimbal by setting it to primary control
        //https://mavlink.io/en/messages/common.html#MAV_CMD_DO_GIMBAL_MANAGER_CONFIGURE
        //mavlink command : MAV_CMD_DO_GIMBAL_MANAGER_CONFIGURE (1001)
        //parameter 1 : system id of ros node (1)
        //parameter 2 : component id of ros node (1)
...
void LandingTest::land() {
  ...
  /*calculate pitch and yaw for gimbal stabilization for GIMBAL_MANAGER_PITCHYAW*/
  Eigen::Vector3f down_world(0, 0, -1);
  Eigen::Vector3f camera_x = q.conjugate()*down_world;  
  camera_x.normalize();
  Eigen::Matrix3f r = q.toRotationMatrix();
  Eigen::Vector3f relative_down(cos(asin(-R(2, 0))), -sin(asin(-R(2, 0))), 0.0f);
  Eigen::Vector3f camera_y = camera_x.cross(relative_down).normalized();
  Eigen::Vector3f camera_z = camera_x.cross(camera_y).normalized();

  float horizon = sqrt(camera_x.x()*camera_x.x() + camera_x.y()*camera_x.y());
  float desired_pitch_ = atan2(-camera_x.z(), horizon)*rad_to_deg;
  float desired_yaw_ = atan2(camera_x.y(), camera_x.x())*rad_to_deg;
  /*************************end calculation of pitch yaw***********************/

  //set gimbal pitch & yaw
  //https://mavlink.io/en/messages/common.html#MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW
  //mavlink message : MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW (1000)
  //parameter 1 : desired pitch of gimbal
  //parameter 2 : desired yaw of gimbal
  //parameter 3 : pitch rate(deg/s)
  //parameter 4 : yaw rate(deg/s)
  ...
  //can confirm gimbal stays down
  //need roll control for a fixed camera frame

  /***test line for gimabl_manager_set_attitude***/
...
/********publish gimbal_manager_set_attitude************/
//https://mavlink.io/en/messages/common.html#GIMBAL_MANAGER_SET_ATTITUDE
//https://docs.px4.io/main/en/msg_docs/GimbalManagerSetAttitude
void LandingTest::publish_gimbal_set_attitude(Eigen::Quaternionf q, float ang_vel_x, float ang_vel_y, float ang_vel_z) {
  ...
  msg.angular_velocity_x = ang_vel_x; //roll rate
  msg.angular_velocity_y = ang_vel_y; //pitch rate
  msg.angular_velocity_z = ang_vel_z; //yaw rate
  msg.flags = 0; //bitmask for control modes
  msg.target_system = 1; //gimbal manager system id, 1 for sim
  msg.target_component = 1; //gimbal manager componenet id, 1 for sim
  msg.gimbal_device_id = 154; //gimbal device id, 0 for all gimbals, 1 ~ 6 for non-mavlink gimbals, 154 as default for mavlink gimbal
  msg.timestamp = this->get_clock()->now().nanoseconds() / 1000;
